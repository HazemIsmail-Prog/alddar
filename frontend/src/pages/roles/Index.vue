...
const abilities = computed(() => {
  return {
    ...
    canView: can('view role'),
    showActions: canAny(['edit role', 'delete role']),
  }
})
...
onMounted(() => {
  ...
  axiosInstance.get('/api/roles/getRelatedLists')
  .then(response => {
...
<template>
  ...
  <Form :apiUrl="apiUrl" :permissions="permissions" />
  ...
  <SidebarLayout>
        ...
        <template #header>
            <div class="flex justify-between items-center flex-1"> 
              ...
              <Button v-if="abilities.canCreate" variant="default" size="sm" @click="handleOpenForm()">Create Role</Button>
            </div>
        </template>
        ...
        <Table v-if="records.length > 0" class="mt-4">
            ...
            <TableBody>
              <TableRow v-for="role in records" :key="role.id">
                  <TableCell>{{ role.name }}</TableCell>
                  <TableCell class="w-full">
                    <div class="flex flex-wrap gap-2">
                      <Badge variant="outline" v-for="permission in role.permissions" :key="permission.id">
                        {{ permission.name }}
                      </Badge>
                    </div>
                  </TableCell>
                  <TableCell v-if="abilities.showActions">
                      <div class="flex gap-2 justify-end">
                        <Edit v-if="abilities.canEdit" @click="handleOpenForm(role)" class="size-5 cursor-pointer" />
                        <Trash v-if="abilities.canDelete && isDeleting !== role.id" class="size-5 cursor-pointer text-red-500 dark:text-red-400" @click="deleteRecord(apiUrl, role.id)" />
                        <Badge variant="destructive" v-if="isDeleting === role.id">
                          <Spinner /> Deleting...
                        </Badge>
                      </div>
                  </TableCell>
